javascript: (function () {
            ...
            function getAdDiv() {
                ...
                if (playerRootDiv != null) {
                    ...
                    if (adDiv == null) {
                        ...
                        adDiv.innerHTML =
                            '<div class="player-ad-notice" style="color: white; background-color: rgba(0, 0, 0, 0.8); position: absolute; top: 0px; left: 0px; padding: 10px;"><p></p></div>';
                        adDiv.style.display = "none";
    ...
    async function processM3U8(url, textStr, realFetch) {
        ...
        if (haveAdTags) {
            ...
            if (
                OPT_MODE_NOTIFY_ADS_WATCHED &&
                !streamInfo.IsMidroll &&
                (streamInfo.BackupFailed || streamInfo.BackupUrl != null)
            ) {
                ...
                if (!streamInfo.BackupFailed && streamInfo.BackupUrl == null) {
                    ...
                    if (accessTokenResponse.status === 200) {
                        ...
                        if (encodingsM3u8Response.status === 200) {
                            ...
                            console.log(
                                "Backup url request (encodingsM3u8) failed with " + encodingsM3u8Response.status
                            );
                        }
                    } else {
        ...
        try {
            ...
            if (matches.length > 1) {
                ...
                var podPosition = parseInt(
                    attr["X-TV-TWITCH-AD-POD-POSITION"] ? attr["X-TV-TWITCH-AD-POD-POSITION"] : "0"
                );
